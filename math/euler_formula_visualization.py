...
def create_euler_animation():
    ...
    # 创建箭头和点的对象
    arrow = FancyArrowPatch((0, 0), (1, 0), color='#4169E1', arrowstyle='->', mutation_scale=15, linewidth=2)
    ax1.add_patch(arrow)
    point, = ax1.plot([], [], 'ro', markersize=8)
    angle_text = ax1.text(0.2, 0.2, '', bbox=dict(facecolor='white', alpha=0.8))
    ...
    # 公式数值显示
    formula_text = ax1.text(-1.45, 1.2, '', fontsize=13, color='black', bbox=dict(facecolor='white', alpha=0.8))
    
    # 简明解释文本
    explain_text = ax1.text(-1.45, 1.35, '', fontsize=15, color='#008B8B', bbox=dict(facecolor='white', alpha=0.8))
    ...
    def update(frame):
        ...
        # 公式数值联动
        formula_text.set_text(f"e^{{iθ}} = cos(θ) + i·sin(θ)\n= {x:.2f} + i·{y:.2f}")
